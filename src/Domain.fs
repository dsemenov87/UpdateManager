...
module Requirements =
  ...
  type MAptekaProfile =
    | MAptekaProfile of string
  
  type MAptekaRestriction =
    | NoRestriction
    | Exactly of MAptekaProfile
    
  type MAptekaResolver = MAptekaProfile -> Set<MAptekaProfile>

  [<RequireQualifiedAccess>]
  // To make reasoning and writing tests easier.
  // Ideally we would "simplify" the trees to a "normal" form internally
  // [<CustomEquality; CustomComparison>]
  type MAptekaRestrictionP =
    private
    | ExactlyP of MAptekaProfile
    | AtLeastP of MAptekaProfile * MAptekaResolver
    // Means: Take all frameworks NOT given by the restriction
    | NotP of MAptekaRestrictionP
    | OrP of MAptekaRestrictionP list
    | AndP of MAptekaRestrictionP list

    override this.ToString() =
      match this with
      | MAptekaRestrictionP.ExactlyP r -> "== " + r.ToString()
      | MAptekaRestrictionP.AtLeastP (r, _) -> ">= " + r.ToString()
      | MAptekaRestrictionP.NotP(MAptekaRestrictionP.AtLeastP (r, _)) -> sprintf "< " + r.ToString()
      | MAptekaRestrictionP.NotP(fr) -> sprintf "NOT (%O)" fr
      | MAptekaRestrictionP.OrP(frl) ->
          match frl with
          | [] -> "false"
          | [single] -> sprintf "%O" single
          | _ -> sprintf "|| %s" (System.String.Join(" ", frl |> Seq.map (sprintf "(%O)")))
      | MAptekaRestrictionP.AndP(frl) ->
          match frl with
          | [] -> "true"
          | [single] -> sprintf "%O" single
          | _ -> sprintf "&& %s" (System.String.Join(" ", frl |> Seq.map (sprintf "(%O)")))
    
    /// The list represented by this restriction (ie the included set of MApteka)
    // NOTE: All critical paths test only if this set is empty, so we use lazy seq here
    // member x.RepresentedMAptekas =
    //   match x with
    //   | MAptekaRestrictionP.ExactlyP r -> [ r ] |> Set.ofList
    //   | MAptekaRestrictionP.AtLeastP (r, f) -> f r
    //   | _ -> Set.empty
    //   // | MAptekaRestrictionP.NotP(fr) ->
    ...
    // override x.Equals(y) = (match y with :? MAptekaRestrictionP as r -> r.RepresentedMAptekas = x.RepresentedMAptekas | _ -> false)
    // override x.GetHashCode() = x.RepresentedMAptekas.GetHashCode()
    // interface System.IComparable with
    //   member x.CompareTo(y) = (match y with :? MAptekaRestrictionP as r -> compare x.RepresentedMAptekas r.RepresentedFrameworks | _ -> failwith "wrong type")
